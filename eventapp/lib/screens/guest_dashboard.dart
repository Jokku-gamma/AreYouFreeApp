...
import 'package:intl/intl.dart'; // Add this import for date formatting if needed in AnnouncementCard
import '../services/firebase_service.dart';
import '../models/announcement_model.dart';
import '../models/usermodel.dart'; // Import UserModel
...
class _GuestDashboardScreenState extends State<GuestDashboardScreen> {
  ...
  UserModel? _currentUser; // Change to UserModel?
  ...
  void _handleGuestResponse(Announcement announcement, String response) async {
    ...
    if (_currentUser?.uid == null) { // Use _currentUser?.uid
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    try {
      await _firebaseService.updateGuestResponse(
        announcement.id,
        _currentUser!.uid, // Use _currentUser!.uid
        response,
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Text(
                                    'Welcome, ${_currentUser?.displayName ?? 'Guest'}!',
